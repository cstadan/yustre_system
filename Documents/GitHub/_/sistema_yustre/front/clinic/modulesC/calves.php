...
require_once '../controllersC/calves_controller.php';
...
<div class="main-content" id="mainContent">
    <div class="welcome-section">
        <h2 class="welcome-title"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="26" height="26" fill="none" style="vertical-align:middle;margin-right:8px"><ellipse cx="12" cy="14" rx="6" ry="5" stroke="var(--title-gold)" stroke-width="1.8"/><path d="M8 14c0-4 8-4 8 0" stroke="var(--title-gold)" stroke-width="1.8" stroke-linecap="round"/><path d="M9 9V6M15 9V6" stroke="var(--title-gold)" stroke-width="1.8" stroke-linecap="round"/><circle cx="9" cy="5" r="1" fill="var(--title-gold)"/><circle cx="15" cy="5" r="1" fill="var(--title-gold)"/></svg> Calves Registry</h2>
        <p class="welcome-subtitle">Calves management and control</p>
    </div>
    <div class="filters-section">
        <form method="GET" action="calves.php">
            <div class="row g-3">
                <div class="col-md-4"><label class="filter-label"><svg class="label-icon" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="14" height="14" fill="none"><circle cx="11" cy="11" r="7" stroke="currentColor" stroke-width="1.8"/><path d="M16.5 16.5L21 21" stroke="currentColor" stroke-width="1.8" stroke-linecap="round"/></svg> Search by ID, Corral or Age</label><input type="text" name="search" class="search-input" placeholder="Type to search..." value="<?php echo htmlspecialchars($search); ?>"></div>
                <div class="col-md-2"><label class="filter-label"><svg class="label-icon" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="14" height="14" fill="none"><rect x="3" y="3" width="18" height="18" rx="2" stroke="currentColor" stroke-width="1.8"/><path d="M3 9h18M9 9v12" stroke="currentColor" stroke-width="1.8" stroke-linecap="round"/></svg> Filter by Corral</label><select name="corral" class="filter-select"><option value="">All corrals</option><?php foreach ($corrals as $corral): ?><option value="<?php echo htmlspecialchars($corral); ?>" <?php echo $filter_corral==$corral?'selected':''; ?>>Corral <?php echo htmlspecialchars($corral); ?></option><?php endforeach; ?></select></div>
                <div class="col-md-2"><label class="filter-label"><svg class="label-icon" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="14" height="14" fill="none"><circle cx="12" cy="12" r="9" stroke="currentColor" stroke-width="1.8"/><path d="M12 8v4l3 3" stroke="currentColor" stroke-width="1.8" stroke-linecap="round"/></svg> Filter by Status</label><select name="status" class="filter-select"><option value="">All statuses</option><option value="Healthy" <?php echo $filter_status=='Healthy'?'selected':''; ?>>Healthy</option><option value="Sick" <?php echo $filter_status=='Sick'?'selected':''; ?>>Sick</option><option value="Sold" <?php echo $filter_status=='Sold'?'selected':''; ?>>Sold</option><option value="Deceased" <?php echo $filter_status=='Deceased'?'selected':''; ?>>Deceased</option></select></div>
                <div class="col-md-2"><label class="filter-label"><svg class="label-icon" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="14" height="14" fill="none"><path d="M4 6h16M4 12h10M4 18h6" stroke="currentColor" stroke-width="1.8" stroke-linecap="round"/></svg> Order by</label><select name="order" class="filter-select"><option value="id_asc" <?php echo $order_by=='id_asc'?'selected':''; ?>>ID (A-Z)</option><option value="id_desc" <?php echo $order_by=='id_desc'?'selected':''; ?>>ID (Z-A)</option><option value="age_asc" <?php echo $order_by=='age_asc'?'selected':''; ?>>Age (Youngest)</option><option value="age_desc" <?php echo $order_by=='age_desc'?'selected':''; ?>>Age (Oldest)</option></select></div>
                <div class="col-md-2 d-flex align-items-end"><button type="submit" class="btn-search">Search</button></div>
            </div>
            <?php if (!empty($search)||!empty($filter_corral)||!empty($filter_status)||$order_by!='id_asc'): ?><div class="mt-3"><a href="calves.php" class="btn-clear">Clear filters</a></div><?php endif; ?>
        </form>
    </div>
    <div class="table-container">
        <div class="table-header">
            <h3 class="table-title">Calves List <span class="total-badge"><?php echo count($calves); ?> records</span></h3>
            <button class="btn-add" data-bs-toggle="modal" data-bs-target="#addCalfModal"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="15" height="15" fill="none"><path d="M12 5v14M5 12h14" stroke="currentColor" stroke-width="2" stroke-linecap="round"/></svg> Add Calf</button>
        </div>
        <?php if (count($calves) > 0): ?>
        <div class="table-scroll"><table class="data-table">
            <thead><tr><th>ID</th><th>Corral</th><th>Status</th><th>Age</th><th>Comments</th><th>Date Registered</th></tr></thead>
            <tbody><?php foreach ($calves as $calf): ?>
            <tr style="cursor:pointer;" onclick="openEditModal('<?php echo htmlspecialchars($calf['id']); ?>','<?php echo htmlspecialchars($calf['corral']); ?>','<?php echo htmlspecialchars($calf['status']); ?>','<?php echo htmlspecialchars($calf['age']); ?>','<?php echo htmlspecialchars(addslashes($calf['comments'])); ?>')">
                <td><strong><?php echo htmlspecialchars($calf['id']); ?></strong></td>
                <td>Corral <?php echo htmlspecialchars($calf['corral']); ?></td>
                <td><?php $s=strtolower($calf['status']);$c='status-healthy';if($s=='sick')$c='status-sick';elseif($s=='sold')$c='status-sold';elseif($s=='deceased')$c='status-deceased'; ?><span class="status-badge <?php echo $c; ?>"><?php echo htmlspecialchars($calf['status']); ?></span></td>
                <td><?php echo htmlspecialchars($calf['age']); ?> mos</td>
                <td><?php echo htmlspecialchars($calf['comments']); ?></td>
                <td><?php echo date('m/d/Y', strtotime($calf['date_register'])); ?></td>
            </tr><?php endforeach; ?>
            </tbody>
        </table></div>
        <?php else: ?><div class="no-data"><div style="display:flex;justify-content:center;margin-bottom:10px;opacity:.5;color:var(--text-muted)"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="38" height="38" fill="none"><circle cx="11" cy="11" r="7" stroke="currentColor" stroke-width="1.5"/><path d="M16.5 16.5L21 21" stroke="currentColor" stroke-width="1.5" stroke-linecap="round"/><path d="M8 11h6M11 8v6" stroke="currentColor" stroke-width="1.5" stroke-linecap="round"/></svg></div>No calves found matching the search criteria</div><?php endif; ?>
    </div>
</div>
...
<div class="modal fade" id="addCalfModal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered"><div class="modal-content">
        <div class="modal-header"><h5 class="modal-title"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="15" height="15" fill="none"><path d="M12 5v14M5 12h14" stroke="currentColor" stroke-width="2" stroke-linecap="round"/></svg> Add New Calf</h5><button type="button" class="btn-close" data-bs-dismiss="modal"></button></div>
        <form method="POST" action="calves.php"><input type="hidden" name="action" value="add_calf"><input type="hidden" name="csrf_token" value="<?php echo $csrf_token; ?>">
            <div class="modal-body p-4"><div class="row g-3">
                ...
                <div class="col-md-6"><label class="modal-label"><svg class="label-icon" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="14" height="14" fill="none"><circle cx="12" cy="12" r="9" stroke="currentColor" stroke-width="1.8"/><path d="M12 8v4l3 3" stroke="currentColor" stroke-width="1.8" stroke-linecap="round"/></svg> Status <span class="text-danger">*</span></label><select name="status" class="modal-input" required><option value="">Select...</option><option value="Healthy">Healthy</option><option value="Sick">Sick</option><option value="Sold">Sold</option><option value="Deceased">Deceased</option></select></div>
                <div class="col-md-6"><label class="modal-label"><svg class="label-icon" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="14" height="14" fill="none"><rect x="3" y="4" width="18" height="18" rx="2" stroke="currentColor" stroke-width="1.8"/><path d="M16 2v4M8 2v4M3 10h18" stroke="currentColor" stroke-width="1.8" stroke-linecap="round"/></svg> Age (months) <span class="text-danger">*</span></label><input type="number" name="age" class="modal-input" placeholder="e.g. 3" min="0" max="36" required></div>
            ...
            <div class="modal-footer"><button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cancel</button><button type="submit" class="btn-save">Save Calf</button></div>
        </form>
    </div></div>
</div>
...
<div class="modal fade" id="editCalfModal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered"><div class="modal-content">
        <div class="modal-header"><h5 class="modal-title"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="15" height="15" fill="none"><path d="M11 4H4a2 2 0 00-2 2v14a2 2 0 002 2h14a2 2 0 002-2v-7" stroke="currentColor" stroke-width="1.8" stroke-linecap="round"/><path d="M18.5 2.5a2.121 2.121 0 013 3L12 15l-4 1 1-4 9.5-9.5z" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"/></svg> Edit Calf &mdash; <span id="editModalTitle"></span></h5><button type="button" class="btn-close" data-bs-dismiss="modal"></button></div>
        <form method="POST" action="calves.php"><input type="hidden" name="action" value="edit_calf"><input type="hidden" name="csrf_token" value="<?php echo $csrf_token; ?>"><input type="hidden" name="edit_id" id="edit_id">
            <div class="modal-body p-4"><div class="row g-3">
                <div class="col-md-6"><label class="modal-label"><svg class="label-icon" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="14" height="14" fill="none"><rect x="3" y="3" width="18" height="18" rx="2" stroke="currentColor" stroke-width="1.8"/><path d="M3 9h18M9 9v12" stroke="currentColor" stroke-width="1.8" stroke-linecap="round"/></svg> Corral <span class="text-danger">*</span></label><input type="text" name="corral" id="edit_corral" class="modal-input" required></div>
                <div class="col-md-6"><label class="modal-label"><svg class="label-icon" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="14" height="14" fill="none"><circle cx="12" cy="12" r="9" stroke="currentColor" stroke-width="1.8"/><path d="M12 8v4l3 3" stroke="currentColor" stroke-width="1.8" stroke-linecap="round"/></svg> Status <span class="text-danger">*</span></label><select name="status" id="edit_status" class="modal-input" required><option value="Healthy">Healthy</option><option value="Sick">Sick</option><option value="Sold">Sold</option><option value="Deceased">Deceased</option></select></div>
                <div class="col-md-6"><label class="modal-label"><svg class="label-icon" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="14" height="14" fill="none"><rect x="3" y="4" width="18" height="18" rx="2" stroke="currentColor" stroke-width="1.8"/><path d="M16 2v4M8 2v4M3 10h18" stroke="currentColor" stroke-width="1.8" stroke-linecap="round"/></svg> Age (months) <span class="text-danger">*</span></label><input type="number" name="age" id="edit_age" class="modal-input" min="0" max="36" required></div>
            ...
            <div class="modal-footer d-flex justify-content-between">
                <button type="button" class="btn btn-danger" onclick="deleteCalf()"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="15" height="15" fill="none"><path d="M3 6h18M8 6V4h8v2M19 6l-1 14H6L5 6" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"/></svg> Delete</button>
...
<form method="POST" action="calves.php" id="deleteForm"><input type="hidden" name="action" value="delete_calf"><input type="hidden" name="csrf_token" value="<?php echo $csrf_token; ?>"><input type="hidden" name="delete_id" id="delete_id"></form>
